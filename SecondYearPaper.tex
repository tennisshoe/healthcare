...
Most of the current literature on this topic focuses on who are the entrepreneurs that are prevented from starting firms. Holtz-Eakin et al.\footnote{HOLTZ-EASKIN}, Wellington \footnote{WELLINGTON} and Fairie et al \footnote{Fairie et all} consider entrepreneurs with health care through their spouses. Fairie et al \footnote{Fairie et all} study entrepreneurs near the traditional retirement age. Heim and Lurie show variation in health insurance's impact by age group and tax filing status. 

However we are not aware of research asking the complementary question of what \textit{enterprises} are blocked from creation. When probed about the entrepreneurial impact of Massachusetts' 2006 health insurance market reform, one high-tech entrepreneur in the Kendell Square area responded:
\begin{quote}
...
\end{quote}
The above quote suggests health insurance reform would have a greater impact on enterprises requiring low capital investment. We may for example see an effect in self-employed consultants but not in more high growth startups. More generally, the impact of reform may vary by industry and not just entrepreneur. 

In this paper we explore how enterprises are affected by access to health care. We contribute to the literature in two ways. We first present simple models that suggest capital intensity and non-profit status should create heterogeneity in outcomes by industry. We then test those model predictions using the 2006 Massachusetts health reform as a natural experiment. Our results complement previous research by suggesting that even if the propensity to become self-employed increases, access to health care may not matter much for the wider economy. We do not find evidence that access to health care increases firm creation.  

The rest of the article proceeds as follows. Section \ref{sec:review} provides an overview of the previous research done in this area. Section \ref{sec:reform} describes the Massachusetts health reform we use as a policy shock. Section \ref{sec:data} describes the data sources we use in our analysis. Section \ref{sec:model} outlines a model that considers how the reform should heterogeneously affect entrepreneurship by industry. Section \ref{sec:results} presents the effects of the reform on entrepreneurship and Section \ref{sec:conclude} concludes. 
...
care changes increase incentives to leave current employment. Early work by Gruber and Madrain found that enabling
employees to keep their current policy reduces job lock \footnote{Gruber and Madrain}. More recently, Garthwait, Gross and Notowidigo show how the loss of healthcare to a group in Tennesee increased
the area's labor supply \footnote{Garthwait, Gross and Notowidigo}.
...
To summarize, although there is uncertainty about the impact of health insurance on entrepreneurship, there is strong evidence that both job lock exists and that the Massachusetts reform changed health care in the state. 
...
Note that all three of our data sources suffer from being measurements of net rather than gross values. If  we do not obverse a change in number establishments over a period of time, this does not necessarily mean no new firms were created. No change happens when the rate of firm entrance equals the rate of firm exit. Exit and entrance are also definitional; a previously existent firm that hired its first worker would be listed as a entrant in the CBP data. Unfortunately we were not able to find a satisfactory public data set with gross establishment or firm numbers. However we think any change in the rate of entrepreneurship will still appear in our dataset. Figure \ref{fig:construct} shows how the number of nonemployers the construction industry, typically very cyclical, rose during the boom period of mid-2000 only to fall again with the 2008 recession. 
