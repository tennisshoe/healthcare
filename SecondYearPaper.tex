...
\title{Does Access to Healthcare Increase Entrepreneurship?}
\author{Ankur Chavda}
...
How important is access to health insurance for an entrepreneur's decision to start a new venture? Traditionally health insurance in the US has been tied to employment with side effects that increase the relative costs for individuals attempting purchase health insurance on their own. This has led to the phenomena of job lock that has been documented to for example delay employee retirement \cite{gruberMadrian}. Job lock due to health insurance may also be dissuading some employees from creating new ventures. As one entrepreneur writes:
\begin{quote}
...
\begin{quote}
[The] top barrier[s] to people starting companies are [a] lack of drive and lack of capital ... it takes a lot of drive to start companies, and lack of healthcare is too small a barrier compared to others.  
\end{quote}

Previous research on whether access to health insurance affects entrepreneurial rates has produced mixed results. The Affordable Care Act of 2010 required health insurance plans to cover dependents up to the age of 26. Bailey uses this policy change to test whether improving access to health insurance increases entrepreneurial rates and finds no effect. Heim and Lurie analyze tax filing data using the 2006 health care reform in Massachusetts as a shock which increased access to health insurance. They find a small decrease in the overall probability a taxpayer earns a majority of their income from self-employment. Tuzemen and Becker apply census data to the same shock and find that self-employment rates in Massachuetts rose relative to other New England states. 

We add to this debate in a number of ways. First we expand the work done studying the Massachusetts health reform by considering establishment creation rates as an alternative measure of entrepreneurship. Second we provide a model that relates entrepreneurial rates to industry capital requirements. Finally we test whether marginal entrepreneurship is sensitive to changes in expected tax rates. 

The rest of the article proceeds as follows. Section 2 describes the Massachusetts health reform we use as a policy shock.  Section 3 outlines a model that considers how the reform should heterogeneously affect entrepreneurship by industry. Section 4 describes the data sources we use in our analysis. Section 5 presents the effects of the reform on entrepreneurship and Section 6 concludes. 
